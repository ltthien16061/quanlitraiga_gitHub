...
public class CheckLoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            if (wrappedRequest.getRequestURI().startsWith(wrappedRequest.getContextPath() + ResourceHandler.RESOURCE_IDENTIFIER)) {
                chain.doFilter(wrappedRequest, wrappedResponse);
            } else if (wrappedRequest.getRequestURL().toString().endsWith("login.xhtml")) {
                chain.doFilter(wrappedRequest, wrappedResponse);
            } else if (accountMB.isLoggedIn()) {
                chain.doFilter(wrappedRequest, wrappedResponse);
            } else {
                StringBuffer url = wrappedRequest.getRequestURL();
                String uri = wrappedRequest.getRequestURI();
                String ctx = wrappedRequest.getContextPath();
                String base = url.substring(0, url.length() - uri.length() + ctx.length());
                wrappedResponse.sendRedirect(base + "/faces/login.xhtml");
            }
    ...
    class RequestWrapper extends HttpServletRequestWrapper {
        ...
        public void setParameter(String name, String[] values) {
            if (debug) {
                System.out.println("CheckLoginFilter::setParameter(" + name + "=" + values + ")" + " localParams = " + localParams);
            }
        ...
        public String getParameter(String name) {
            if (debug) {
                System.out.println("CheckLoginFilter::getParameter(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public String[] getParameterValues(String name) {
            if (debug) {
                System.out.println("CheckLoginFilter::getParameterValues(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Enumeration getParameterNames() {
            if (debug) {
                System.out.println("CheckLoginFilter::getParameterNames() localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Map getParameterMap() {
            if (debug) {
                System.out.println("CheckLoginFilter::getParameterMap() localParams = " + localParams);
            }
            if (localParams == null) {
